...
pub fn stop(_args: &Args, clear_env: bool) -> Result<()> {
    ...
    stop_service(&services::service_id(services::LocalServer::ID));
    stop_service(&services::service_id(services::CloudflareTunnel::ID));
    stop_service(&services::service_id(services::LocalDnsServer::ID));
