...
impl State {
    ...
    /// Loads state from an arbitrary linkup directory (e.g. an instance under `~/.linkup/instances/N`).
    pub fn load_from_dir(dir: &Path) -> anyhow::Result<Self> {
        let state_file_path = dir.join(LINKUP_STATE_FILE);
        let content = fs::read_to_string(&state_file_path)
            .with_context(|| format!("Failed to read state file on {:?}", &state_file_path))?;
...
pub fn config_to_state(
    ...
    let linkup = LinkupState {
        session_name: config.linkup.session_name.clone().unwrap_or_default(),
        ...
        local_server_port: config.linkup.local_server_port,
    };
...
domains:
  ...
  - domain: api.example.com
    ...
    #[test]
    fn test_config_to_state_default_port() {
        let config: linkup::config::Config = serde_yaml::from_str(CONF_STR).unwrap();
        let state = config_to_state(config, "config.yaml".to_string(), false);
        assert_eq!(state.linkup.local_server_port, None);
    }

    #[test]
    fn test_config_to_state_with_local_server_port() {
...
domains:
  - domain: example.com
    default_service: frontend
        ...
        let config: linkup::config::Config = serde_yaml::from_str(config_with_port).unwrap();
        let state = config_to_state(config, "config.yaml".to_string(), false);
        assert_eq!(state.linkup.local_server_port, Some(9080));
    }

    #[test]
    fn test_state_serialization_roundtrip_with_port() {
        let config: linkup::config::Config = serde_yaml::from_str(
            r#"
linkup:
  worker_url: https://remote-linkup.example.com
  worker_token: test_token_123
...
domains:
  - domain: example.com
    default_service: frontend
        ...
        let deserialized: State = serde_yaml::from_str(&yaml_string).unwrap();
        assert_eq!(deserialized.linkup.local_server_port, Some(9080));
    }

    #[test]
    fn test_config_to_state_without_session_name() {
        let config: linkup::config::Config = serde_yaml::from_str(CONF_STR).unwrap();
        let state = config_to_state(config, "config.yaml".to_string(), false);
        assert_eq!(state.linkup.session_name, "");
    }

    #[test]
    fn test_config_to_state_with_session_name() {
        let config_with_name = r#"
linkup:
  worker_url: https://remote-linkup.example.com
  worker_token: test_token_123
...
domains:
  - domain: example.com
    default_service: frontend
        ...
        let config: linkup::config::Config = serde_yaml::from_str(config_with_name).unwrap();
        let state = config_to_state(config, "config.yaml".to_string(), false);
        assert_eq!(state.linkup.session_name, "rafael2");
    }

    #[test]
    fn test_state_deserialization_without_port_field() {
        let state_yaml = r#"
linkup:
  session_name: test
...
services: []
        ...
        let state: State = serde_yaml::from_str(state_yaml).unwrap();
        assert_eq!(state.linkup.local_server_port, None);
    }

    #[test]
    fn test_session_name_priority_config_wins() {
        let config_with_name = r#"
linkup:
  worker_url: https://remote-linkup.example.com
  worker_token: test_token_123
  session_name: from-config
services:
  - name: frontend
    remote: http://remote-service1.example.com
    local: http://localhost:8000
domains:
  - domain: example.com
    default_service: frontend
        "#;
        let config: linkup::config::Config = serde_yaml::from_str(config_with_name).unwrap();
        let mut state = config_to_state(config, "config.yaml".to_string(), false);

        let previous_name = "from-previous-state".to_string();
        if state.linkup.session_name.is_empty() {
            state.linkup.session_name = previous_name;
        }
        assert_eq!(state.linkup.session_name, "from-config");
    }

    #[test]
    fn test_session_name_priority_falls_back_to_previous() {
        let config: linkup::config::Config = serde_yaml::from_str(CONF_STR).unwrap();
        let mut state = config_to_state(config, "config.yaml".to_string(), false);

        assert!(state.linkup.session_name.is_empty());

        let previous_name = "from-previous-state".to_string();
        if state.linkup.session_name.is_empty() {
            state.linkup.session_name = previous_name;
        }
        assert_eq!(state.linkup.session_name, "from-previous-state");
    }

    #[test]
    fn test_load_from_dir_success() {
        let tmp =
            std::env::temp_dir().join(format!("linkup-test-load-from-dir-{}", std::process::id()));
...
"#;
        ...
        let state = State::load_from_dir(&tmp).unwrap();
        assert_eq!(state.linkup.session_name, "my-session");
        assert_eq!(
            state.linkup.worker_url,
            Url::parse("https://remote-linkup.example.com").unwrap()
        );
    ...
    #[test]
    fn test_load_from_dir_no_state_file() {
        let tmp = std::env::temp_dir().join(format!(
            "linkup-test-load-from-dir-missing-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).unwrap();

        let result = State::load_from_dir(&tmp);
