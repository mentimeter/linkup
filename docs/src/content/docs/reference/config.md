...
Optional path (relative to your project) where Linkup looks for `.env.*.linkup` files when you run `linkup start`. The contents are appended into the matching `.env.*` file (e.g. `.env.development.linkup` → `.env.development`) and removed by `linkup stop`. See [Environment variables for local services](/linkup/guides/configure#environment-variables-for-local-services).
